module.exports = (client, message) => {
    ...
    if (cmd && DJ.enabled && DJ.commands.includes(cmd.name)) {
        ...
        if (!message.member._roles.includes(roleDJ.id)) {
            return message.channel.send(`M PHẢI CÓ ROLE ĐƯỢC TIN TƯỞNG Của tao chưa  ${message.author}`);
        }
    }

    if (cmd && cmd.voiceChannel) {
        if (!message.member.voice.channel) return message.channel.send(`? ${message.author}`);
